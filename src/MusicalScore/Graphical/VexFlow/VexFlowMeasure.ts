...
export class VexFlowMeasure extends GraphicalMeasure {
    ...
    public static readonly HasMeasureLocalCorrectNotePositions: boolean = true;
    public static readonly HasIntrinsicTickCorrection: boolean = true;
        ...
        for (const voiceID in this.beams) {
            if (this.beams.hasOwnProperty(voiceID)) {
                ...
                for (const beam of this.beams[voiceID]) {
                    ...
                    if (this.canCreateVexFlowBeam(notes)) {
    ...
    private canCreateVexFlowBeam(notes: StemmableNote[]): boolean {
        const quarterTicks: number = VF.durationToTicks("4");
        return notes.length > 1 && notes.every((note: StemmableNote) => {
            // VexFlow 1.x returns a number at runtime although its declaration
            // exposes Fraction; newer builds can return the declared object.
            const ticks: VF.Fraction | number = note.getIntrinsicTicks();
            return (typeof ticks === "number" ? ticks : ticks.value()) < quarterTicks;
        });
    }
                    ...
                    if (!currentTuplet) {
                        ...
                        if (currentTuplet !== noteTuplet) { // new tuplet, finish old one
                            if (tupletNotesToAutoBeam.length > 1) {
                                ...
                                if (this.canCreateVexFlowBeam(beamVFNotes)) {
